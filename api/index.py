...
FIELDS=["lead_score","lead_tier","verification_status","confidence","property_address","property_city","property_state","property_zip","owner_name","mailing_address","last_transfer_date","last_sale_price","property_type","acreage","assessed_land_value","assessed_improvement_value","parcel_id","lowtax_match_status","lowtax_current_account_balance","lowtax_fall_balance_due","lowtax_fall_tax","lowtax_spring_balance_due","lowtax_spring_tax","lowtax_pay_year","lowtax_status","lowtax_tax_type","lowtax_photo","source","screening_reasons"]
...
def enrich_lowtax(rows:list[dict[str,Any]], max_lookups:int=40)->dict[str,Any]:
    """Enrich a bounded set of GIS rows using LowTaxInfo owner search + parcel-key join."""
    cache={}; lookups=0; matched=0
    # Start with records most useful to wholesale screening: rows already having owner + parcel.
    for row in rows:
...
def research_data(limit:int,city:str|None,sort:str="best",strategy:str="all",min_score:int=0,property_type:str="all",owner_type:str="all",min_years:int=0,tax_signal:str="all",absentee:str="all"):
    ...
    # First pass establishes the candidate pool. LowTaxInfo then enriches that pool
    # before the final two-pass QC so tax/owner evidence can participate in scoring.
    ...
    # Rebuild the final input from enriched candidates; second_pass is the delivery gate.
    result=second_pass(candidates)
