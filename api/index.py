...
app = FastAPI(title="Wholesale AI Agent", version="0.7.0")
...
def _live_result(limit: int, city: str | None, sort: str = "best") -> Dict[str, Any]:
    where = "1=1"
    if city:
    ...
    if sort == "lowest":
        leads.sort(key=lambda x: (x.get("lead_score", 0), x.get("confidence", 0), x.get("property_address", "")))
    else:
        leads.sort(key=lambda x: (-x.get("lead_score", 0), -x.get("confidence", 0), x.get("property_address", "")))
    result["screening"] = {"pool_scanned": len(rows), "returned": min(limit, len(leads)), "method": "rank full bounded pool, sort requested order, then return leads", "sort": sort}
...
<section class='hero'><h2>Find better wholesale leads.</h2><p>Scan public parcel data, rank the opportunity signals, and run a second-pass quality check before a lead reaches your list.</p><div class='controls'><input id='city' class='input' value='South Bend' placeholder='City (optional)'><select id='limit' class='select'><option value='10'>Top 10</option><option value='25' selected>Top 25</option><option value='50'>Top 50</option><option value='100'>Top 100</option></select><select id='sort' class='select'><option value='best' selected>Best rated → lowest</option><option value='lowest'>Lowest rated → highest</option></select><button class='btn' onclick='runSearch()'>Run Research</button></div></section>
...
<section class='panel'><h3>Quick access</h3><p>Run a fresh screen or download a ranked report.</p><div class='actions'><a class='secondary' href='/api/live-leads?limit=25&city=South%20Bend&sort=best'>View API results</a><a class='secondary' href='/api/report.csv?limit=25&city=South%20Bend&sort=best'>Download CSV</a><a class='secondary' href='/api'>System status</a></div></section>
...
async function runSearch(){const city=document.getElementById('city').value.trim();const limit=document.getElementById('limit').value;const sort=document.getElementById('sort').value;const summary=document.getElementById('summary');const box=document.getElementById('results');summary.textContent='Scanning public parcel data and running QC…';box.innerHTML='<div class="empty">Research in progress…</div>';let url='/api/live-leads?limit='+encodeURIComponent(limit)+'&sort='+encodeURIComponent(sort)+(city?'&city='+encodeURIComponent(city):'');try{const r=await fetch(url);const data=await r.json();if(data.status!=='ok')throw new Error(data.error||'Research failed');const result=data.result||{};const leads=result.leads||[];summary.textContent=`Scanned ${result.screening?.pool_scanned??0} records • ${leads.length} leads • ${sort==='best'?'best rated first':'lowest rated first'}`;if(!leads.length){box.innerHTML='<div class="empty">No ranked leads returned.</div>';return}box.innerHTML=leads.map((x,i)=>`<article class="lead"><div class="leadhead"><div><div class="address">${i+1}. ${esc(x.property_address||'Unknown address')}</div><div class="owner">${esc(x.owner_name||'Unknown owner')}</div></div><div class="score">${esc(x.lead_score??0)}/100</div></div><div class="meta"><span class="tag">Tier ${esc(x.lead_tier||'C')}</span><span class="tag">${esc(x.verification_status||'unverified')}</span>${x.last_transfer_date?`<span class="tag">Transfer ${esc(x.last_transfer_date)}</span>`:''}${x.property_type?`<span class="tag">${esc(x.property_type)}</span>`:''}</div><div class="reasons">${esc((x.screening_reasons||[]).join(' • ')||'No screening reasons recorded.')}</div></article>`).join('')}catch(e){summary.textContent='Research failed';box.innerHTML='<div class="empty">'+esc(e.message)+'</div>'}}
</script></body></html>"""
...
def live_leads(limit: int = Query(25, ge=1, le=100), city: str | None = Query(None, max_length=40), sort: str = Query("best", pattern="^(best|lowest)$")):
    try:
        return {"status": "ok", "source": "South Bend/St. Joseph County public ArcGIS parcel layer", "query": {"city": city, "limit": limit, "sort": sort}, "result": _live_result(limit, city, sort)}
    except Exception as exc:
        return {"status": "error", "error": str(exc), "hint": "The public GIS service may be temporarily unavailable."}

@app.get("/api/report.csv")
def report_csv(limit: int = Query(25, ge=1, le=100), city: str | None = Query(None, max_length=40), sort: str = Query("best", pattern="^(best|lowest)$")):
    try:
        result = _live_result(limit, city, sort)
        content = _csv_text(result)
        filename = f"wholesale_leads_{(city or 'st_joseph_county').lower().replace(' ', '_')}_{limit}_{sort}.csv"
